...
/* Analysis:
I have made this mistake multiple times: The adjacency list we declare is a vector of size numCourses (i.e. the number of vertices in the 
graph). Therefore on line 48, we should have vector<unordered_set<int>> adj(numCourses). I have forgotten to include the size when initialing
this vector several times now:(
*/

// Submission #2: DFS cycle detection: Accepted
class Solution {
public:
    vector<int> findOrder(int numCourses, vector<pair<int, int>>& prerequisites) {
        vector<int> order;
